...
export async function arenaRoutes(server: FastifyInstance) {
  ...
  const buildVoteSummary = async (arenaRoomId: string, userId?: string) => {
    const votes = await prisma.arenaVote.groupBy({
      by: ['winnerAgentId'],
      where: { arenaRoomId },
    ...
    if (userId) {
      const vote = await prisma.arenaVote.findUnique({
        where: { arenaRoomId_voterId: { arenaRoomId, voterId: userId } },
      });
  ...
  // Get vote summary for an arena match
  server.get(
    '/:id/votes',
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      const summary = await buildVoteSummary(room.id, userId);
      return { summary };
    }
  );

  // Vote for a winner in an arena match
  server.post(
    '/:id/vote',
    { preHandler: [authenticate] },
    async (
      request: FastifyRequest<{ Params: { id: string }; Body: { winnerAgentId: string } }>,
      reply: FastifyReply
    ) => {
      try {
        const room = await prisma.arenaRoom.findUnique({
        ...
        if (!['completed', 'cancelled'].includes(room.status)) {
          return reply.status(400).send({ error: 'Voting is only available after the arena ends' });
        }

        const winnerAgentId = request.body?.winnerAgentId;
        if (!winnerAgentId) {
          return reply.status(400).send({ error: 'Winner agent is required' });
        }

        const participantIds = new Set(room.participants.map((p) => p.agentId));
        if (!participantIds.has(winnerAgentId)) {
          return reply.status(400).send({ error: 'Winner must be a participant' });
        }

        await prisma.arenaVote.upsert({
          where: {
            arenaRoomId_voterId: {
              arenaRoomId: room.id,
        ...
        const summary = await buildVoteSummary(room.id, request.user.id);
        return { summary };
      } catch (err) {
        console.error('Error voting on arena:', err);
